...
def LSTM_Uni(X_train, y_train, X_test, y_test, word_index, embedding_matrix, max_len, seed):
    ...
    for i,(train, test) in enumerate(kfold):
        print("Fit fold", i+1," ==========================================================================")
        model_info=model.fit(X_train[train], y_train[train], epochs=2, batch_size=32, validation_data=(X_train[test], y_train[test]),
...
# GRU model
def GRU_model(X_train, y_train, X_test, y_test, word_index, embedding_matrix, max_len, seed):
    earlystop = EarlyStopping(monitor='val_loss', min_delta=0, patience=5, verbose=1, mode='auto')
    callbacks_list = [earlystop]
    model = Sequential()
